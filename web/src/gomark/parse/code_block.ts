...
export function NewCodeBlockParser(): CodeBlockParser {
...
export class CodeBlockParser {
  Match(tokens: Token[]): [Node | null, number] {
    ...
    for(const row of (rows.slice(1))) {
      if (row.length === 3 && row[0].Type === TokenType.Backtick && row[1].Type === TokenType.Backtick && row[2].Type === TokenType.Backtick) {
        matched = true
        break
      }
