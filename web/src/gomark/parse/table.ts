...
export function NewTableParser(): TableParser {
...
export class TableParser {
  Match(tokens: Token[]): [Node | null, number] {
    ...
    for (let index = 0; index < tokenizerAfrterDelimiter.length; index++) {
      ...
      if (
        (
          delimiterTokens[0].Type !== TokenType.Colon &&
          delimiterTokens[0].Type !== TokenType.Hyphen
        ) ||
        (
...
function matchTableCellTokens(tokens: Token[]): [number, boolean] {
  ...
  if (cells.l !== 0 || cells[cells.length - 1].length !== 0) {
    return [0, false]
  }
