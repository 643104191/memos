...
export function findPrevSiblingExceptLineBreak(node: Node | null): Node | null {
  ...
  if (
    prev !== null &&
    prev.Type() === NodeType.LineBreakNode &&
    prev.PrevSibling() !== null &&
    (prev.PrevSibling() as Node).Type() !== NodeType.LineBreakNode
  ) {
...
export function findNextSiblingExceptLineBreak(node: Node | null): Node | null {
  ...
  if (
    next !== null &&
    next.Type() === NodeType.LineBreakNode &&
    next.NextSibling() !== null &&
    (next.NextSibling() as Node).Type() !== NodeType.LineBreakNode
  ) {
